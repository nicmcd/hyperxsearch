...
BRANCH=cmake
BUILD_SPEC="hyperxsearch@${BRANCH} target=${SPACK_TARGET}"
...
spack spec ${BUILD_SPEC} > /tmp/hyperxsearch_spec_${BRANCH}

# Extracts the exact spec of a dependency then returns the install directory
function location() {
    local spec=$(grep "\^${1}@" /tmp/hyperxsearch_spec_${BRANCH} | sed 's/\s.*^//g')
...
add_to_install_rpath $(location libgrid)/lib
echo "INSTALL_RPATH:"
echo "$INSTALL_RPATH"
echo ""
...
add_to_prefix_path $(location libgrid)
echo "PREFIX_PATH:"
echo "$PREFIX_PATH"
echo ""
